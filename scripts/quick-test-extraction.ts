 ...
 * Uses short timeouts to fail fast
 */
...
async function quickTest() {
  console.log('🚀 Quick PDF Extraction Test\n');
  ...
  // 3. Check if workers can process jobs
  const queue = getQueue(QUEUE_NAMES.DOCUMENT_PROCESSING);
  ...
  // 4. Create a test document in database
  ...
  // 6. Wait for job to complete (with timeout)
  ...
  while (Date.now() - startTime < timeout) {
    ...
    if (jobState === 'completed') {
      ...
      if (updatedDoc) {
        console.log(`📄 Extraction status: ${updatedDoc.extraction_status}`);
        console.log(`📝 Text extracted: ${updatedDoc.extracted_text?.length || 0} characters`);
        
        if (updatedDoc.extracted_text && updatedDoc.extracted_text.length > 0) {
      ...
      console.log(`\n📋 Obligations created: ${obligations?.length || 0}`);
