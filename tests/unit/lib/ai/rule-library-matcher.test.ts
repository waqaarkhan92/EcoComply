/**
 * rule-library-matcher Tests
 * Comprehensive tests for lib/ai/rule-library-matcher.ts
 * Target: 100% coverage
 */
...
// TODO: Import functions from 'lib/ai/rule-library-matcher.ts'

describe('rule-library-matcher', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
 ...
 *
 * Steps to complete:
 * 1. Review lib/ai/rule-library-matcher.ts and identify all exported functions
 ...
 * 7. Run: npm test -- rule-library-matcher.ts
 * 8. Check coverage: npm run test:coverage -- rule-library-matcher.ts
