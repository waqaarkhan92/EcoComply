...
export async function PUT(
  ...
  const { data: existingTest, error: fetchError } = await supabaseAdmin
    ...
    if (body.nox_result !== undefined) updateData.nox_result = body.nox_result ? Number(body.nox_result) : undefined;
    if (body.so2_result !== undefined) updateData.so2_result = body.so2_result ? Number(body.so2_result) : undefined;
    if (body.co_result !== undefined) updateData.co_result = body.co_result ? Number(body.co_result) : undefined;
    if (body.particulates_result !== undefined) updateData.particulates_result = body.particulates_result ? Number(body.particulates_result) : undefined;
