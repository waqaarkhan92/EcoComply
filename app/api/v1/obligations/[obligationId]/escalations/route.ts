/**
 * Obligation Escalations Endpoint
 * GET /api/v1/obligations/{obligationId}/escalations - Get escalations for obligation
 */
...
export async function GET(
  ...
  const { limit, cursor } = parsePaginationParams(request);
    ...
    let query = supabaseAdmin
      .from('escalations')
      .select('id, escalation_type, reason, escalated_at, resolved_at, status, created_at')
      .eq('entity_type', 'obligations')
    ...
    if (sort.length > 0) {
      ...
      // Default sort by escalated_at descending
  ...
  const { data: escalations, error } = await query;
    ...
    console.error('Get obligation escalations error:', error);
    return errorResponse(
      ErrorCodes.INTERNAL_ERROR,
